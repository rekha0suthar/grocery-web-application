...
export default {
    ...
    setup() {
        ...
        const cartProducts = computed(() => store.state.userCartProducts);
        const totalItems = computed(() => store.state.userTotalItems);
        const totalPrice = computed(() => store.state.userTotalPrice);

        onMounted(() => {
            store.dispatch('fetchUserCartProducts')
        })

        const removeFromCart = async (product_id) => {
            await store.dispatch('removeFromCart', { product_id, role: 'user'})               
        };

        const updateQuantity = async (product_id, newQuantity) => {
            await store.dispatch('updateQuantity', { product_id, newQuantity, role: 'user' })
        };

        return { cartProducts, totalItems, totalPrice, removeFromCart, updateQuantity }
    },
    methods: {
        checkout() {
