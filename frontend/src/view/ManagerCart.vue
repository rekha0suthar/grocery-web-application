...
export default {
    ...
    setup() {
        ...
        const cartProducts = computed(() => store.state.managerCartProducts);
        const totalItems = computed(() => store.state.managerTotalItems);
        const totalPrice = computed(() => store.state.managerTotalPrice);

        onMounted(() => {
            store.dispatch('fetchManagerCartProducts')
        })

        const removeFromCart = async (product_id) => {
            await store.dispatch('removeFromCart', { product_id, role: 'manager'})               
        };

        const updateQuantity = async (product_id, newQuantity) => {
            await store.dispatch('updateQuantity', { product_id, newQuantity, role: 'manager' })
        };

        const checkout = () => {
            router.push('/store-manager/checkout')
        }
