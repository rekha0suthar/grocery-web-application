<template>
    <div class="cart-container" style="margin-top: 350px;">
        <div class="pay-card">
            ...
            <button class="btn btn-primary" @click="completeOrder(userId)">Complete order</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    mounted() {
        axios.get('/user/checkout', { headers: { Authorization: `Bearer ${localStorage.access_token}` } })
            .then(response => {
    ...
    methods: {
        async completeOrder(userId) {
            ...
            if (confirmation) {
                try {
                    await axios.post('/user/orders', {user_id: userId})
